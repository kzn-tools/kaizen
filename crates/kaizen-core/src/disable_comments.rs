...
//! - `// kaizen-disable-next-line Q030` - disable Q030 for the next line
//! - `// kaizen-disable-line Q030` - disable Q030 for the current line
//! - `// kaizen-disable-next-line` - disable all rules for the next line
//! - `// kaizen-disable-line` - disable all rules for the current line
//! - `// kaizen-disable-next-line Q030, Q031` - disable multiple rules
    ...
    pub fn from_source(source: &str) -> Self {
        ...
        for (line_idx, line) in source.lines().enumerate() {
            ...
            if let Some(comment_start) = line.find("//") {
                ...
                if let Some(rest) = comment.strip_prefix("kaizen-disable-next-line") {
                    let rule_ids = parse_rule_ids(rest);
                    let target_line = line_num + 1;
                    directives.add(DisableDirective::new(target_line, rule_ids));
                } else if let Some(rest) = comment.strip_prefix("kaizen-disable-line") {
                    let rule_ids = parse_rule_ids(rest);
...
"#;
    ...
    fn disable_line_with_specific_rule() {
        let source = r#"
var x = 1; // kaizen-disable-line Q030
"#;
        let directives = DisableDirectives::from_source(source);
...
"#;
    ...
    fn disable_next_line_multiple_rules() {
        let source = r#"
// kaizen-disable-next-line Q030, Q033
var x = 1;
"#;
        let directives = DisableDirectives::from_source(source);
    ...
    fn disable_line_multiple_rules() {
        let source = r#"
var x = 1; // kaizen-disable-line Q030, Q033
"#;
        let directives = DisableDirectives::from_source(source);
...
"#;
    ...
    fn directive_on_first_line() {
        let source = r#"// kaizen-disable-next-line Q030
var x = 1;"#;
        let directives = DisableDirectives::from_source(source);
...
"#;
    ...
    fn similar_but_not_directive() {
        let source = r#"
// kaizen-disable Q030
// kaizen-disable-block Q030
// some kaizen-disable-next-line comment
var x = 1;
"#;
        let directives = DisableDirectives::from_source(source);
