...
#[command(
    ...
    author,
    version,
    about = "Language Server Protocol implementation for Lynx",
    long_about = "Lynx LSP provides language server protocol support for JavaScript and TypeScript.\n\n\
...
mod tests {
    ...
    fn cli_parses_default_arguments() {
        let cli = Cli::try_parse_from(["lynx-lsp"]).unwrap();
    ...
    fn cli_parses_log_file() {
        let cli = Cli::try_parse_from(["lynx-lsp", "--log-file", "/tmp/lynx.log"]).unwrap();
        assert_eq!(cli.log_file, Some(PathBuf::from("/tmp/lynx.log")));
    }
