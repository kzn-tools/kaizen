...
use lynx_core::diagnostic::{Diagnostic, Fix, FixKind};
use lynx_core::rules::{RuleCategory, RuleRegistry, Severity};
use serde::Serialize;
...
impl<'a> SarifFormatter<'a> {
            ...
            runs: vec![SarifRun {
                tool: SarifTool {
                    driver: SarifDriver {
                        ...
                        information_uri: Some("https://github.com/mpiton/lynx"),
                        rules,
                    },
                },
                results,
    ...
    fn convert_result(&self, diag: &Diagnostic) -> SarifResult {
        ...
        let properties =
            if diag.confidence != lynx_core::rules::Confidence::High || diag.suggestion.is_some() {
...
mod tests {
    use super::*;
    use lynx_core::rules::Confidence;
    ...
    fn format_includes_tool_info() {
        ...
        assert_eq!(driver["name"], "Lynx");
        assert!(driver["semanticVersion"].is_string());
        assert_eq!(driver["informationUri"], "https://github.com/mpiton/lynx");
    }
    ...
    fn format_includes_fix_when_present() {
        ...
        let diag = Diagnostic::new("Q030", Severity::Warning, "Avoid var", "test.js", 1, 0)
            .with_end(1, 3)
            .with_fix(lynx_core::diagnostic::Fix::replace(
                "Replace with let",
                "let",
                1,
                0,
