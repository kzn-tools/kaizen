//! Configuration loading and parsing for Lynx
//!
//! Provides functionality to load and parse `lynx.toml` configuration files.
...
"#,
    ...
    fn config_error_display_is_helpful() {
        let err = ConfigError::ParseError {
            path: PathBuf::from("/path/to/lynx.toml"),
            message: "expected `=`".to_string(),
        };

        let msg = format!("{}", err);

        assert!(msg.contains("/path/to/lynx.toml"));
